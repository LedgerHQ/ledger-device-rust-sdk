...
/*                                                                         */
/* ----------------------------------------------------------------------- */
cx_err_t cx_groestl(cx_hash_t *hash, uint32_t mode, const uint8_t *in, size_t len, uint8_t *out, size_t out_len) {
  size_t sz = 0;
...
bool cx_groestl_validate_context(const cx_groestl_t *ctx) {
  ...
  const struct hashState_s *state       = &ctx->ctx;

  if ((output_size != 224 / 8) && (output_size != 256 / 8) && (output_size != 384 / 8) && (output_size != 512 / 8)) {
    return 0;
  }
  if (ctx->output_size < state->hashlen) {
    return 0;
  }

  if ((state->hashlen > 512 / 8) || (state->buf_ptr >= state->statesize) || (state->columns > COLS1024) ||
      (state->rounds > 14) || (state->statesize > SIZE1024)) {
    return 0;
  }
  if (state->statesize < LENGTHFIELDLEN) {
    return 0;
  }

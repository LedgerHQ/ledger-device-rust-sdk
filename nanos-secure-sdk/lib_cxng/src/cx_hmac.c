...
static cx_hash_t *cx_get_hash_ctx(cx_hmac_t *ctx) {
  if (ctx == NULL) {
    return NULL;
  }
  if (cx_is_allowed_digest(cx_get_algorithm(ctx))) {
    return &ctx->hash_ctx;
  }
  return NULL;
}
...
cx_err_t cx_hmac_update(cx_hmac_t *ctx, const uint8_t *data, size_t data_len) {
  ...
  if (((hash_ctx = cx_get_hash_ctx(ctx)) == NULL) ||
      (data == NULL)) {
    return CX_INVALID_PARAMETER;
  }
...
// --------------------------------------------------------------------------
#ifdef HAVE_RIPEMD160
cx_err_t cx_hmac_ripemd160_init_no_throw(cx_hmac_ripemd160_t *hmac, const uint8_t *key, unsigned int key_len) {

...
#define CX_MAX_TRIES 100
  ...
  if (key == NULL ||
      !CX_CURVE_RANGE(key->curve, WEIERSTRASS) ||
 ...
 RETRY:
...
#ifdef HAVE_ECC_ADDITIVE_SPLITTING
  // Additive splitting with random projective coordinates
  CX_CHECK(cx_ecpoint_rnd_scalarmul(&Q, rnd, domain_length));
#else
  CX_CHECK(cx_ecpoint_scalarmul(&Q, rnd, domain_length));
#endif // HAVE_ECC_ADDITIVE_SPLITTING
  ...
  // s = k_inv*u
  //
  CX_CHECK(cx_bn_rand(&v));
  CX_CHECK(cx_bn_reduce(&t, &v, &n));

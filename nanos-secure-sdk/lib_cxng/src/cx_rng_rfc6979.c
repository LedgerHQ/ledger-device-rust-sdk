...
/*                                                                         */
/* ----------------------------------------------------------------------- */
static void cx_rfc6979_shr(uint8_t *x, size_t x_len, uint32_t n) {
  uint8_t ri_1, ri;
  uint32_t j;
...
static size_t cx_rfc6979_bits2int(cx_rnd_rfc6979_ctx_t *rfc_ctx, const uint8_t *b, size_t b_len, uint8_t *b_out) {
  ...
  if (b_len>rfc_ctx->q_len) {
    l = b_len>>3;
    memcpy(b_out, b,l);
    cx_rfc6979_shr(b_out, l, b_len-rfc_ctx->q_len);
  }
  else  {
    b_len = b_len>>3;
    l = rfc_ctx->r_len>>3;
    memset(b_out, 0, l-b_len);
    memmove(b_out+(l-b_len), b, b_len);
  }

...
impl<'a, O> ParseCfg<'a, O> {
    ...
    pub const fn new(handlers: &'a [Handler<O>], map: fn(Tag) -> u64) -> Self {
        Self { handlers, tag_to_flag: map, common: None }
    }
}
...
pub fn parse<'a, O>(cfg: &ParseCfg<'a, O>, payload: &'a [u8], tlv_out: &mut O) -> Result<()> {
    ...
    while off < payload.len() {
        ...
        off += len;
        let data = TlvData{ tag, value: val, raw };
        if let Some(f) = cfg.common { 
            if !f(&data, tlv_out)? { 
                break; 
            } 
        }
        let h = cfg.handlers.iter().find(|h| h.tag == tag).ok_or(Error::UnknownTag)?;
...
impl<'a> TlvData<'a> {
    ...
    pub fn as_u64_be(&self) -> Result<u64> {
        match self.value.len() {
            ...
            4 => Ok(u32::from_be_bytes([self.value[0], self.value[1], self.value[2], self.value[3]]) as u64),
    ...
    pub fn as_bool(&self) -> Result<bool> { if self.value.len()!=1 { return Err(Error::LengthOverflow);} Ok(self.value[0]!=0) }
    /// Get the value as a UTF-8 string
    pub fn as_str(&self) -> Result<&'a str> { core::str::from_utf8(self.value).map_err(|_| Error::LengthOverflow) }
    /// Get the value as a byte slice with bounded length
    pub fn as_bounded(&self, min: usize, max: usize) -> Result<&'a [u8]> { if self.value.len()<min || self.value.len()>max { return Err(Error::LengthOverflow);} Ok(self.value) }
}
